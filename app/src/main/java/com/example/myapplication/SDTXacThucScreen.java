            ...
            public void onClick(View v) {
                ...
                if (!phoneNumber.isEmpty()) {
                    if (phoneNumber.length() == 9) {
                        ...
                        giangVienRef.orderByChild("sdt").equalTo("+84" + phoneNumber).addListenerForSingleValueEvent(new ValueEventListener() {
                            @Override
                            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                                if (dataSnapshot.exists()) {
                                    // Số điện thoại tồn tại, gửi mã xác thực
                            ...
                            public void onCancelled(@NonNull DatabaseError databaseError) {
                                progressBar.setVisibility(View.GONE);
                                gui_ma_xac_thuc.setVisibility(View.VISIBLE);
                                Toast.makeText(SDTXacThucScreen.this, "Có lỗi xảy ra", Toast.LENGTH_SHORT).show();
                            }
                        });
                    } else {
